...
class CollaborationService {
  ...
  setupNamespaces() {
    ...
    this.videoNamespace.on('connection', (socket) => this.handleVideoConnection(socket));
    
    // Collaborative whiteboard namespace
    this.whiteboardNamespace = this.io.of('/whiteboard');
    this.whiteboardNamespace.on('connection', (socket) => this.handleWhiteboardConnection(socket));
    
    // Code collaboration namespace
    this.codeNamespace = this.io.of('/code');
    this.codeNamespace.on('connection', (socket) => this.handleCodeConnection(socket));
    
    // Chat and general communication
    this.chatNamespace = this.io.of('/chat');
    this.chatNamespace.on('connection', (socket) => this.handleChatConnection(socket));
  }
  ...
  setupEventHandlers() {
    ...
    this.redis.subscribe('collaboration:whiteboard', 'collaboration:code', 'collaboration:video');
  }
  ...
  handleWhiteboardConnection(socket) {
    ...
    socket.on('draw', async (data) => {
      try {
        ...
        await this.redis.publish('collaboration:whiteboard', JSON.stringify({
          type: 'draw',
  ...
  handleCodeConnection(socket) {
    ...
    socket.on('code-change', async (data) => {
      try {
        ...
        await this.autoSaveCodeFile(socket.sessionId, socket.fileId, transformedChange);
        
        // Publish for cross-server sync
        await this.redis.publish('collaboration:code', JSON.stringify({
          type: 'code-change',
  ...
  async validateSessionAccess(sessionId, userId) {
    try {
      const session = await Session.findById(sessionId)
        .populate('participants.userId', 'personal.name account.role');
  ...
  async loadWhiteboardState(sessionId) {
    try {
      const operations = await this.redis.lrange(`whiteboard:${sessionId}`, 0, -1);
  ...
  async loadCodeFileContent(sessionId, fileId) {
    try {
      ...
      return content ? JSON.parse(content) : { content: '', language: 'javascript' };
    } catch (error) {
  ...
  async getVectorClock(roomId) {
    ...
    await this.redis.setex(`vector-clock:${roomId}`, 3600, JSON.stringify(currentClock));
    return currentClock;
  }
  ...
  handleVideoDisconnection(socket) {
    if (socket.sessionId && socket.userId) {
      ...
      logger.info(`User ${socket.userId} left video session ${socket.sessionId}`);
    }
  }
  ...
  handleChatDisconnection(socket) {
    if (socket.sessionId && socket.userId) {
      ...
      logger.info(`User ${socket.userId} left chat in session ${socket.sessionId}`);
    }
  }
