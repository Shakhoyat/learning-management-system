...
const SessionSchema = new mongoose.Schema({
  ...
  materials: {
    shared: [{
      ...
      type: {
        type: String,
        enum: ['document', 'image', 'video', 'audio', 'code', 'link', 'other'],
        required: true
      },
  ...
  analytics: {
    ...
    technicalIssues: [{
      type: {
        type: String,
        enum: ['audio', 'video', 'connection', 'whiteboard', 'recording', 'other']
      },
      description: String,
      timestamp: {
        type: Date,
...
SessionSchema.virtual('isUpcoming').get(function() {
  return this.schedule.startTime > new Date() && ['scheduled', 'confirmed'].includes(this.status);
});
...
SessionSchema.virtual('isPast').get(function() {
  return this.schedule.endTime < new Date() || 
         ['completed', 'cancelled', 'no_show_teacher', 'no_show_learner', 'technical_issue'].includes(this.status);
});

SessionSchema.virtual('canBeCancelled').get(function() {
  const hoursUntilSession = (this.schedule.startTime - new Date()) / (1000 * 60 * 60);
...
SessionSchema.methods.updateStatus = function(newStatus, userId = null) {
  ...
  } else if (['completed', 'cancelled', 'technical_issue'].includes(newStatus) && !this.analytics.actualEndTime) {
    this.analytics.actualEndTime = now;
    if (this.analytics.actualStartTime) {
      this.analytics.actualDuration = (now - this.analytics.actualStartTime) / (1000 * 60); // minutes
    }
  }
...
SessionSchema.statics.getSessionHistory = function(userId, page = 1, limit = 20) {
...
SessionSchema.statics.getSessionAnalytics = function(userId, startDate, endDate) {
