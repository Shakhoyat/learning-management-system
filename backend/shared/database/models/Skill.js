...
const SkillSchema = new mongoose.Schema({
  ...
  relatedSkills: [{
    ...
    relationship: {
      type: String,
      enum: ['complementary', 'advanced', 'alternative', 'foundational', 'specialized'],
      required: true
    },
    strength: {
      type: Number,
  ...
  resources: [{
    type: {
      type: String,
      enum: ['article', 'video', 'book', 'course', 'documentation', 'practice'],
      required: true
    },
    title: String,
    url: String,
...
SkillSchema.index({ name: 'text', description: 'text', tags: 'text', keywords: 'text' });
...
SkillSchema.methods.getPrerequisiteChain = async function() {
  ...
  const buildChain = async (skillId) => {
    ...
    const skill = await this.constructor.findById(skillId).populate('prerequisites.skillId');
...
SkillSchema.methods.getRelatedSkillsGraph = async function(depth = 2) {
  ...
  const buildGraph = async (skillId, currentDepth) => {
    ...
    const skill = await this.constructor.findById(skillId).populate('relatedSkills.skillId');
    if (skill) {
