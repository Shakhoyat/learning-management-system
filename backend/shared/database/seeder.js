...
const skillsData = [
  {
    ...
    description: 'Learn modern JavaScript programming including ES6+, async programming, and frameworks',
...
const seedDatabase = async () => {
  try {
    // Connect to database
    await mongoose.connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/lms_development');
    ...
    const createdSkills = await Skill.insertMany(skillsData.map(skill => ({
      ...skill,
      statistics: {
    ...
    const jsSkill = createdSkills.find(s => s.name === 'JavaScript Programming');
    const reactSkill = createdSkills.find(s => s.name === 'React Development');
    const pythonSkill = createdSkills.find(s => s.name === 'Python Programming');
    ...
    for (let userData of usersData) {
      ...
      userData.skills.learning = randomLearningSkills.map(skill => ({
        ...
        preferredLearningStyle: ['visual', 'auditory', 'kinesthetic', 'reading_writing'][Math.floor(Math.random() * 4)]
      }));
    ...
    for (let i = 0; i < 10; i++) {
      const teacher = createdUsers[Math.floor(Math.random() * createdUsers.length)];
      const learner = createdUsers[Math.floor(Math.random() * createdUsers.length)];
      
      if (teacher._id.toString() !== learner._id.toString() && teacher.skills.teaching.length > 0) {
        const teachingSkill = teacher.skills.teaching[Math.floor(Math.random() * teacher.skills.teaching.length)];
        const startTime = new Date(Date.now() + Math.random() * 30 * 24 * 60 * 60 * 1000); // Random time in next 30 days
        ...
        sessionsData.push({
          ...
          title: `Learning ${createdSkills.find(s => s._id.toString() === teachingSkill.skillId.toString())?.name}`,
          ...
          room: {
            videoRoomId: `room_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
